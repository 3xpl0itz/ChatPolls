...
public class CHPStartRewardModifyPrompt extends StringPrompt{

	private MainChatPolls plugin;
	private EachPoll currPoll = null;
	private Player currPlayer;
	
	public CHPStartRewardModifyPrompt(MainChatPolls plugin, EachPoll currentPoll, Player pl)
	{
		this.plugin = plugin;
	...
	public Prompt acceptInput(ConversationContext context, String input) 
	{
		...
		try
		{
			for (int currReward = 0; currReward < currPoll.getRewards().size(); currReward++)
			{
				...
				{
					//We made it...all checks passed!
					//Lets pass it to Part 2 :)
					return new CHPStartRewardModifyPromptP2(plugin, currPoll, currPlayer, (currReward+1));
				}
			}
		}
	...
	public String getPromptText(ConversationContext context) 
	{
		...
		output += "Enter the number of the command you'd like to modify.";
		return output;
	}
