...
public class CHPStartModifyPrompt extends StringPrompt{
	...
	public CHPStartModifyPrompt(MainChatPolls plugin, EachPoll currentPoll, Player pl)
	{
		this.plugin = plugin;
		...
		try
		{
			for (int currReward = 0; currReward < currPoll.getRewards().size(); currReward++)
			{
				if (Integer.parseInt(input) == (currReward+1))
				{
					...
					return new CHPStartModifyPromptP2(plugin, currPoll, currPlayer, (currReward+1));
				}
			}
		}
